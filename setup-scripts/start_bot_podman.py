...
def find_task_layout_file(path):
    ...
    raise FileNotFoundError(
        f"Missing task layout file in {path}. Consider naming your layout file: task_layout.json"
    )
...
def main(args):
    ...
    if args.dev is True:
        ...
        # if args.copy_images is True:
        #     # images must be all placed in the assets/images directory
        #     target_dir = Path("../slurk/slurk/views/static/assets/images/")
        #     if not target_dir.exists():
        #         target_dir.mkdir(parents=True)
        #     images_path = Path(f"{bot_base_path}/assets/images")

        #     if not images_path.exists():
        #         raise FileNotFoundError("Your bot is missing an 'assets/images' directory")
        ...
        # build image
        subprocess.run(
            ["podman", "build", "--tag", "slurk/server", "-f", "Dockerfile", "."],
            cwd=Path("../slurk"),
        )
    ...
    if args.copy_plugins_to_container:
        print(f"Copying plugins to {args.copy_plugins_to_container}")
        subprocess.run([
            "podman",
            "cp",
            "sailsman/plugins/.",
            f"{args.copy_plugins_to_container}:/usr/src/slurk/views/static/plugins/"
        ])
    if args.copy_images_to_container:
        print(f"Copying images to {args.copy_images_to_container}")
        subprocess.run([
            "podman",
            "cp",
            "sailsman/assets/images/.",
            f"{args.copy_images_to_container}:/usr/src/slurk/views/static/assets/images/"
        ])
    ...
    podman_args.append(f"slurk/{bot_name}")
    result = subprocess.run(podman_args, capture_output=True, text=True)
    print(f"CONTAINER_ID: {result.stdout}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--copy-images",
        help="copy all the files in the assets/images directory to slurk's assets/images before starting the slurk server",
        action="store_true",
    )
    parser.add_argument(
        "--copy-plugins-to-container",
        help="copy all the files in the plugins directory to slurk's plugins of the specified running container before starting the slurk server",
    )
    parser.add_argument(
