...
def main(args):
    ...
    if args.dev is True:
        ...
        if args.copy_to_server:
            plugins_path = Path(f"{bot_base_path}/plugins")
            image_name = args.copy_to_server
            ...
            if not container_id:
                raise FileNotFoundError(f"Docker image {image_name} does not exist")

            print(f"bot base path: {bot_base_path.name}")
            for filename in plugins_path.iterdir():
                subprocess.run(
                    ["docker", "cp", str(filename), f"{container_id}:/usr/src/slurk/views/static/plugins/"]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--copy-to-server",
        help="copy the specified image to the server before starting the slurk server",
    )
